...
static const uint32_t WORDS_PER_BUFFER = ADS131_FRAMES_PER_BUFFER * ADS131_WORDS_PER_FRAME;
...
static void __not_in_flash_func(ads131_dma_irq_handler)(void) {
    ...
    if (dma_channel_get_irq0_status(g_dma_chan_a)) {
        dma_channel_acknowledge_irq0(g_dma_chan_a);
        
        // 1. Mark Buffer A as full for Main loop
        g_buffers.buffer_full[0] = true;
        g_buffers.total_frames += ADS131_FRAMES_PER_BUFFER;

        // 2. RE-ARM Channel A (ready for chaining back from B)
        // Set address and count without triggering (last param: false).
        dma_channel_set_write_addr(g_dma_chan_a, g_buffers.frames[0], false);
        dma_channel_set_trans_count(g_dma_chan_a, WORDS_PER_BUFFER, false);
    }
    
    if (dma_channel_get_irq0_status(g_dma_chan_b)) {
        dma_channel_acknowledge_irq0(g_dma_chan_b);

        // 1. Mark Buffer B as full
        g_buffers.buffer_full[1] = true;
        g_buffers.total_frames += ADS131_FRAMES_PER_BUFFER;

        // 2. RE-ARM Channel B (ready for chaining back from A)
        dma_channel_set_write_addr(g_dma_chan_b, g_buffers.frames[1], false);
        dma_channel_set_trans_count(g_dma_chan_b, WORDS_PER_BUFFER, false);
    }
}
...
void ads131_start_pio_dma_capture(void) {
    ...
    float target_pio_freq = 100000000.0f; // 100 MHz
    float div = (float)sys_hz / target_pio_freq;
